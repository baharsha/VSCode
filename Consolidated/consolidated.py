    ...
    for date in dates:
        ...
        for m in models:
            if m['type'] == 'image':
                ...
                if val > 0:
                    data.append({"TimeStamp": date, "Model": m['name'], "Type": "image", "Metric": "GeneratedImages", "Value": val, "Deployment": m['dept'], "ResourceId": "/subscriptions/sub/resourceGroups/rg/providers/Microsoft.CognitiveServices/accounts/demo-project-a", "Account": "demo-project-a"})
            else:
                prompts = int(random.gauss(500, 100) * hour_mod)
                gens = int(random.gauss(200, 50) * hour_mod)
                if prompts > 0:
                    data.append({"TimeStamp": date, "Model": m['name'], "Type": m['type'], "Metric": "ProcessedPromptTokens", "Value": prompts, "Deployment": m['dept'], "ResourceId": "/subscriptions/sub/resourceGroups/rg/providers/Microsoft.CognitiveServices/accounts/demo-project-a", "Account": "demo-project-a"})
                    data.append({"TimeStamp": date, "Model": m['name'], "Type": m['type'], "Metric": "GeneratedTokens", "Value": gens, "Deployment": m['dept'], "ResourceId": "/subscriptions/sub/resourceGroups/rg/providers/Microsoft.CognitiveServices/accounts/demo-project-a", "Account": "demo-project-a"})
    
    # Fake Cost Data for Demo (SEK)
    cost_data = []
    unique_days = sorted(list(set([d.date() for d in dates])))
    for d in unique_days:
        # Resource 1: Demo Project A
        cost_data.append({
            "Date": pd.to_datetime(d),
            "ActualCost": random.uniform(100, 300),
            "OriginalCurrency": "SEK",
            "ResourceId": "/subscriptions/sub/resourceGroups/rg/providers/Microsoft.CognitiveServices/accounts/demo-project-a"
        })
        # Resource 2: Demo Project B (No traffic, just cost)
        cost_data.append({
            "Date": pd.to_datetime(d),
            "ActualCost": random.uniform(20, 80),
            "OriginalCurrency": "SEK",
            "ResourceId": "/subscriptions/sub/resourceGroups/rg/providers/Microsoft.CognitiveServices/accounts/demo-project-b"
        })
...
if fetch_btn:
    ...
    if not df_raw.empty:
        ...
        if not df_cost.empty:
            ...
            # Add ResourceName for visualization
        ...
        with tab_cost:
            ...
            # 1. Agg Usage/Carbon per Project (Account)
            df_usage_agg = df_raw.groupby('Account').agg(
            ...
            # 2. Agg Cost per Project (ResourceName)
            df_cost_agg = pd.DataFrame()
            if not df_cost.empty:
                df_cost_agg = df_cost.groupby('ResourceName').agg(
                    Cost_SEK=('CostSEK', 'sum')
                ).reset_index()
            
            # 3. Merge Cost and Usage
            # If cost data exists, we merge. If not, we just show usage.
            if not df_cost_agg.empty:
                df_project_summary = pd.merge(df_cost_agg, df_usage_agg, left_on='ResourceName', right_on='Account', how='outer')
                # Fill NAs
                df_project_summary['ResourceName'] = df_project_summary['ResourceName'].fillna(df_project_summary['Account'])
                df_project_summary = df_project_summary.drop(columns=['Account']).fillna(0)
            else:
                df_project_summary = df_usage_agg.rename(columns={'Account': 'ResourceName'})
                df_project_summary['Cost_SEK'] = 0.0

            # --- TOP KPIs MATCHING IMAGE ---
            kpi_c1, kpi_c2, kpi_c3, kpi_c4 = st.columns(4)
            
            total_spend = df_project_summary['Cost_SEK'].sum()
            avg_monthly = total_spend # Simplified if range < 30 days
            
            kpi_c1.metric("Average Monthly Cost", f"{total_spend:,.0f} SEK") # Placeholder, technically need date range math
            ...
            with c_chart:
                if not df_cost.empty:
                    ...
                    # Update layout to look like monthly bars if daily data
